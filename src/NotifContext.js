...
export let NotifProvider = ({children}) => {
    ...
    let setNotifications = () => {
        if (auth.getUser()?.id != null) {
            fetch("https://ugdymoistaigosbe.herokuapp.com/api/userorders/" + auth.getUser()?.id, {method: 'GET', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${auth.getToken()}` }})
