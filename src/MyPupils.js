    ...
    return (
        <>
            ...
            {loading ? (<div style = {{height:"50vw", width: "100%", display:"flex", justifyContent:"center", alignItems:"center"}}>Turinys kraunasi <span className="material-symbols-outlined">hourglass_top</span></div>) :
                (orders.length > 0 ? <table className = "p-4 w-100" style={{zIndex:"900"}}>
                    ...
                    <tr id = "adminrow" className = "m-4" key = {order.id}> 
                        ...
                        <td>{order.class}</td> 
                        <td><input className="btn btn-danger btn-sm m-2 rounded-0" type = "button" onClick = {() => {setDeletionId(deletionId=order.id); setShowDeleteModal(true); disableScroll()}} value = "Trinti mokinį"></input></td>
